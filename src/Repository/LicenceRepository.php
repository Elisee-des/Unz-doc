...
class LicenceRepository extends ServiceEntityRepository
{
    ...
    public function findAllLicences($idFiliere)
    {

        $connexion = $this->_em->getConnection();
        $requette = "SELECT * FROM `licence` WHERE filiere_id=:idFiliere";
        $resultat = $connexion->executeQuery($requette, ["idFiliere" => $idFiliere]);
        $donnees = $resultat->fetchAllAssociative();

        return ["licencesDeCeDepartement" => $donnees];
