...
class FiliereRepository extends ServiceEntityRepository
{
    ...
    public function findAllFilieres($idDepartement)
    {

        $connexion = $this->_em->getConnection();
        $requette = "SELECT * FROM `filiere` WHERE departement_id=:idDepartement";
        $resultat = $connexion->executeQuery($requette, ["idDepartement" => $idDepartement]);
        $donnees = $resultat->fetchAllAssociative();

        return ["filieresDeCeDepartement" => $donnees];

    }

    public function totalFilieres()
    {
        $connexion = $this->_em->getConnection();
        $requette = "SELECT COUNT(*) totalFilieres  FROM `filiere`";
        $resultat = $connexion->executeQuery($requette);
        $totalFilieres = $resultat->fetchAllAssociative();

        return $totalFilieres[0];
    }
