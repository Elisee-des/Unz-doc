...
class AnneeRepository extends ServiceEntityRepository
{
    ...
    public function remove(Annee $entity, bool $flush = false): void
    {
        ...
        public function findAllAnnees($idOption)
    {

        $connexion = $this->_em->getConnection();
        $requette = "SELECT * FROM `annee` WHERE optionn_id=:idOption";
        $resultat = $connexion->executeQuery($requette, ["idOption" => $idOption]);
        $donnees = $resultat->fetchAllAssociative();

        return ["anneeDeCetteOpion" => $donnees];
    }
