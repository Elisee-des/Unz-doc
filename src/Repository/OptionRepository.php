...
class OptionRepository extends ServiceEntityRepository
{
    ...
    public function findAllOptions($idLicence)
    {

        $connexion = $this->_em->getConnection();
        $requette = "SELECT * FROM `option` WHERE licence_id=:idLicence";
        $resultat = $connexion->executeQuery($requette, ["idLicence" => $idLicence]);
        $donnees = $resultat->fetchAllAssociative();

        return ["optionDeCetteLicence" => $donnees];
    }

    public function totalOptions()
    {
        $connexion = $this->_em->getConnection();
        $requette = "SELECT COUNT(*) totalOptions  FROM `option`";
        $resultat = $connexion->executeQuery($requette);
        $totalOptions = $resultat->fetchAllAssociative();
