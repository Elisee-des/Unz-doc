...
class DepartementRepository extends ServiceEntityRepository
{
    ...
    public function totalDepartements()
    {
        $connexion = $this->_em->getConnection();
        $requette = "SELECT COUNT(*) totalDepartements  FROM `departement`";
        $resultat = $connexion->executeQuery($requette);
        $totalDepartements = $resultat->fetchAllAssociative();

        return $totalDepartements[0];
    }
