...
class ModuleRepository extends ServiceEntityRepository
{
    ...
    public function findAllModules($idAnnee)
    {

        $connexion = $this->_em->getConnection();
        $requette = "SELECT * FROM `module` WHERE annee_id=:idAnnee";
        $resultat = $connexion->executeQuery($requette, ["idAnnee" => $idAnnee]);
        $donnees = $resultat->fetchAllAssociative();

        return ["moduleDeCetteAnnee" => $donnees];
    }

    public function totalModules()
    {
        $connexion = $this->_em->getConnection();
        $requette = "SELECT COUNT(*) totalModules  FROM `module`";
        $resultat = $connexion->executeQuery($requette);
        $totalModules = $resultat->fetchAllAssociative();
